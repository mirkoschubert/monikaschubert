(function(window) {
  ...
  GridLoader.prototype._filter = function(ev) {
    this.filterIds = [];
  ...
  GridLoader.prototype._openItem = function(ev, item) {
    ...
    var win = this._getWinSize(),
      ...
      z = Math.min(
        Math.min(
          win.width * Math.abs(this.options.imgPosition.x) - this.options.pagemargin,
          originalSize.width - this.options.pagemargin
        ) / gImg.offsetWidth,
        Math.min(
          win.height * Math.abs(this.options.imgPosition.y) - this.options.pagemargin,
    ...
    this.items.forEach(function(item) {
      if (item.classList.contains('grid-item--current')) {
        ...
        if (direction === 'prev') {
          ...
          if (self.isFiltered) {
            nextItem =
              self.filterIds.indexOf(i) === 0
                ? self.items[self.filterIds[self.filterIds.length - 1]]
                : self.items[self.filterIds[self.filterIds.indexOf(i) - 1]];
          } else {
          ...
          if (self.isFiltered) {
            nextItem =
              self.filterIds.indexOf(i) < self.filterIds.length - 1
                ? self.items[self.filterIds[self.filterIds.indexOf(i) + 1]]
                : self.items[self.filterIds[0]];
          } else {
  ...
  GridLoader.prototype._setOriginal = function(src) {
    if (!src) {
      ...
      this.originalImg.style.maxWidth =
        'calc(' + parseInt(Math.abs(this.options.imgPosition.x) * 100) + 'vw - ' + this.options.pagemargin + 'px)';
      this.originalImg.style.maxHeight =
        'calc(' + parseInt(Math.abs(this.options.imgPosition.y) * 100) + 'vh - ' + this.options.pagemargin + 'px)';
